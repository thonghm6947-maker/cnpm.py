...
def list_recruiter_applications():
    """
    List applications for recruiter's jobs with full candidate details
    ---
    get:
      ...
      responses:
        200:
          description: List of applications with candidate details
    """
    current_user = request.current_user
    ...
    job_service = get_job_service()
    applications = job_service.get_recruiter_applications_with_details(recruiter.recruiter_id)
...
@cm_recruiter_bp.route('/applications/<int:application_id>', methods=['PUT'])
@token_required
def update_application_status(application_id):
    ...
    ---
    put:
      summary: Update application status (e.g., reviewing, interview, hired, rejected)
      tags:
        - Recruiter Applications
      security:
        - BearerAuth: []
    ...
    if current_user.get('role') != 'recruiter':
        return jsonify({'success': False, 'error': 'Only recruiters can access this'}), 403
    
    data = request.get_json() or {}
    new_status = data.get('status')
    notes = data.get('notes', '')
    
    valid_statuses = ['pending', 'reviewing', 'interview', 'offer', 'hired', 'rejected']
    ...
    # Get recruiter to verify ownership
    recruiter_repo = RecruiterRepository()
    recruiter = recruiter_repo.get_by_user_id(current_user.get('user_id'))
    ...
    job_service = get_job_service()
    result = job_service.update_application_status(
    ...
    if not result:
        return jsonify({'success': False, 'error': 'Application not found or unauthorized'}), 404
...
@cm_recruiter_bp.route('/applications/<int:application_id>', methods=['DELETE'])
@token_required
def delete_application(application_id):
    ...
    ---
    delete:
      summary: Delete an application from recruiter's job
      tags:
        - Recruiter Applications
      security:
        - BearerAuth: []
    ...
    job_service = get_job_service()
    result = job_service.delete_application(
        application_id=application_id,
        recruiter_id=recruiter.recruiter_id
    )
    ...
    if not result.get('success'):
        ...
        return jsonify({'success': False, 'error': result.get('error')}), status_code
